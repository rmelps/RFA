...
class WordsmithChoiceViewController: UIViewController, WordsmithPageViewControllerChild, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        currentTrackRef = Database.database().reference().child("tracks/\(genreRaw)/today")
        userDBRef = Database.database().reference().child("users")
        
        // Find current tracks storage URL
        self.currentTrackRef.observe(.value) { (snapShot: DataSnapshot) in
            let snapVal = snapShot.value as? [String: Any]
            
            if let fileURL = snapVal?["fileURL"] as? String {
                self.trackStorageRef = Storage.storage().reference(forURL: fileURL)
                ...
                if let uid = snapVal?["user"] as? String {
                    ...
                    uidDBRef.observeSingleEvent(of: .value, with: { (snapShot) in
                        ...
                        if let photoPath = snapVal?["photoPath"] as? String {
                            print(photoPath)
                            let ref = Storage.storage().reference(withPath: "profilePics/\(uid)")
    ...
    @IBAction func playPreviewButtonDown(_ sender: MenuButton) {
        ...
        if downloadTask == nil {
            if let player = player {
                ...
                downloadTask = trackStorageRef.getData(maxSize: 15 * 1024 * 1024, completion: { (data, error) in
