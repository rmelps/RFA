...
class StudioViewController: UIViewController {
    ...
    @IBAction func confirmAndUploadTrackButtonTapped(_ sender: UIButton) {
        ...
        let uploadDBRef = Database.database().reference(withPath: "feed/\(genre.rawValue.lowercased())").childByAutoId()
        ...
        let uploadStorageRef = Storage.storage().reference(withPath: "uploads/\(user.uid!)/\(uploadDBRef.key).m4a")
        ...
        let uploadTask = uploadStorageRef.putData(data, metadata: nil) { (meta, error) in
            ...
            if let downloadURL = meta.downloadURL() {
                ...
                uploadDBRef.setValue(track.toAny(), withCompletionBlock: { (error, reference) in
                    ...
                    userDBRef.child(user.uid).observeSingleEvent(of: .value, with: { (snapshot:DataSnapshot) in
                        ...
                        userDBRef.child(user.uid).setValue(val as Any, withCompletionBlock: { (error: Error?, ref: DatabaseReference) in
