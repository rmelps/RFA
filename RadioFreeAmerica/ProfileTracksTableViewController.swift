...
class ProfileTracksTableViewController: UITableViewController {
    ...
    func retrieveRecentTracks(fromUserDBRef ref: DatabaseReference, feedDBRef fref: DatabaseReference, completion: @escaping (([Track]) -> Void)) {
        var allTracks = [Track]()
        ref.observeSingleEvent(of: .value) { (snapshot:DataSnapshot) in
            if let val = snapshot.value as? [String:Any], let tracks = val["tracks"] as? [String] {
                fref.observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
