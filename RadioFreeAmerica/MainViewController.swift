                        ...
                        Auth.auth().signIn(with: credential, completion: { (user:User?, error:Error?) in
                            ...
                            if let firUser = user {
                                ...
                                for profile in firUser.providerData {
                                    ...
                                    var userProf = RFAUser(uid: firUser.uid, name: profile.displayName!, photoPath: String(describing: profile.photoURL!))
                                    AppDelegate.signedInUser = userProf
                                    ...
                                    self.userDBRef.observeSingleEvent(of: .value, with: { (snapShot) in
                                        ...
                                        if snapShot.hasChild(firUser.uid) {
                                            ...
                                            userProf = RFAUser(uid: firUser.uid, snapShot: childSnap, picURL: String(describing: profPic), nameFromProvider: nil)
                                            AppDelegate.signedInUser = userProf
