...
class SettingsViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func doneBarButtonTapped(_ sender: UIBarButtonItem) {
        ...
        AppDelegate.nameDBRef.observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
            ...
            self.updateProfilePicture(withImage: newImage, compressionRatio: 0.65, completion: { (error: Error?) in
                ...
                AppDelegate.userDBRef.child(self.user.uid).observeSingleEvent(of: .value, with: { (snapshot: DataSnapshot) in
                    if var value = snapshot.value as? [String: Any] {
                        value["name"] = newName
                        value["tagLine"] = newTag
                        value["biography"] = newBio
                        
                        AppDelegate.userDBRef.child(self.user.uid).setValue(value as Any, withCompletionBlock: { (error: Error?, ref: DatabaseReference) in
                            if let error = error {
                            ...
                            ref.observeSingleEvent(of: .value, with: { (snapshot:DataSnapshot) in
                                ...
                                if let firUser = Auth.auth().currentUser {
                                    AppDelegate.signedInUser = RFAUser(uid: firUser.uid, snapShot: snapshot, picURL: value?["photoUrl"] as? String, nameFromProvider: nil)
                                }
                            })
                        })
                        self.dismiss(animated: true, completion: nil)
    ...
    func updateProfilePicture(withImage image: UIImage, compressionRatio comp: CGFloat, completion: @escaping ((Error?) -> Void)) {
        ...
        if let data = UIImageJPEGRepresentation(image, comp) {
            AppDelegate.profPicStorRef.child(user.uid).putData(data, metadata: nil) { (meta: StorageMetadata?, error: Error?) in
                if let error = error {
                    completion(error)
                } else {
