...
struct FireAudioManager {
    ...
    static func modifyPostStat(statName name: String, increase: Bool, forTrack track: Track, inGenre genre: GenreChoices) {
        ...
        let feedDBRef = Database.database().reference().child("feed/\(genre.rawValue.lowercased())")
        let ref = feedDBRef.child(key)
        ref.runTransactionBlock({ (thisData: MutableData) -> TransactionResult in
            if var post = thisData.value as? [String: Any], let uid = Auth.auth().currentUser?.uid {
                ...
                return TransactionResult.success(withValue: thisData)
            }
            return TransactionResult.success(withValue: thisData)
            
        }) { (error: Error?, isCommitted: Bool, snapshot: DataSnapshot?) in
            if let error = error {
    ...
    static func removeTrackFromFirebase(track: Track, fromFeedDBRef feedDBRef: DatabaseReference, completion: @escaping (Error?) -> Void) {
        guard let key = track.key else {
        ...
        ref.removeValue { (error: Error?, reference: DatabaseReference) in
            if let error = error {
                completion(error)
                return
            }
            userDBRef.child(track.user).observeSingleEvent(of: .value, with: { (snapshot:DataSnapshot) in
                ...
                userDBRef.child(track.user).setValue(val as Any, withCompletionBlock: { (error:Error?, reference: DatabaseReference) in
                    Storage.storage().reference(forURL: fileURL).delete(completion: { (error: Error?) in
                        if let error = error {
                            completion(error)
                            return
                        }
    ...
    static func loadAndStoreAudioFile(forTrack track: Track, fromFeedDBRef feedDBRef: DatabaseReference, withCompletion finCompletion: @escaping ((AVAudioFile?, Error?) -> Void)) {
        ...
        feedDBRef.child(track.key!).observeSingleEvent(of: .value) { (snapshot: DataSnapshot) in
